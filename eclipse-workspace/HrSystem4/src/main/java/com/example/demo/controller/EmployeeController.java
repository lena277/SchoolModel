...
public class EmployeeController {
	...
	public ResponseEntity<?> approveVacationApprovalsByEmployeeId(@PathVariable Integer id,
		...
		if (vacationApproval == null)
			throw new VacationNotFoundException(approveId);
		...
		List<VacationApproval> app = vacation.getVacationApproval();
		boolean isTrue = true;
		for (int i = 0; i < app.size(); i++)
			if (!app.get(i).isApprove()) {
				isTrue = false;
				break;
			}
	...
	public ResponseEntity<?> rejectVacationApprovalsByEmployeeId(@PathVariable Integer id,
			@PathVariable Integer approveId) {
	...
	public ResponseEntity<?> findVacationRequestsByEmployeeId() {
		...
		return new ResponseEntity<List<Vacation>>(vacationService.getByStatus(false), HttpStatus.OK);
	}
	...
	public ResponseEntity<?> addVacationsByEmployeeId(@PathVariable Integer id, @RequestBody Vacation vacation) {
		...
		for (Employee manager : employee.getManagers()) {
			...
			vacationAppreoval.setVacation(vacation2);
			vacation2.getVacationApproval().add(vacationAppreoval);
	...
	public ResponseEntity<Employee> addEmployee(@RequestBody Employee employee, @PathVariable Integer id) {
		Employee currentEmployee = service.findById(id);
