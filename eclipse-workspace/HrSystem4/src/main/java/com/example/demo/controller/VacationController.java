...
public class VacationController {
	...
	public ResponseEntity<Vacation>  findById(@PathVariable Integer id){
	...
	public ResponseEntity<Employee>  findManagersById(@PathVariable Integer id){
		
		 Vacation currentVacation = service.findById(id);
		 if (currentVacation == null) 
	    	 throw new VacationNotFoundException(id);
	     
	     return new ResponseEntity<Employee>(currentVacation.getEmployee(), HttpStatus.OK);	

		 
		 }
	
	@RequestMapping(value= "/{id}/vacationApprovals" , method = RequestMethod.GET,produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<?>  findVacationApprovalsById(@PathVariable Integer id){
		
		 Vacation currentVacation = service.findById(id);
		 
	     return new ResponseEntity<List<VacationApproval>>(currentVacation.getVacationApproval(), HttpStatus.OK);	
